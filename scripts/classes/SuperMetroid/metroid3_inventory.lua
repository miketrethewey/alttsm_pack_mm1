...
function hasEnergyReserves(amount)
  wantAmount = wantAmount or 0
  local ret = 0
  local hasAmount = getHas("etank") + getHas("rtank")

  if (wantAmount >= hasAmount) then
    ret = hasAmount
  end
...
function canEnterAndLeaveGauntlet()
  ...
  if (((canFly("sm") == 1) or (canHiJump() == 1) or (canDash("sm") == 1)) and (canIbj() == 1) or ((canOpenYellowDoors() == 1) and (has("powerbomb", 2) == 1)) or (has("screw") == 1) or ((canDash("sm") == 1) and (canOpenYellowDoors() == 1) and (hasEnergyReserves(2) == 1))) then
    ret = 1
  end
